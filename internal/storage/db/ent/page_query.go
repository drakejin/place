...
type PageQuery struct {
	...
	withPageSource *PageSourceQuery
...
// QueryPageSource chains the current query on the "page_source" edge.
func (pq *PageQuery) QueryPageSource() *PageSourceQuery {
	query := &PageSourceQuery{config: pq.config}
...
func (pq *PageQuery) Clone() *PageQuery {
	...
	return &PageQuery{
		...
		withPageSource: pq.withPageSource.Clone(),
...
// WithPageSource tells the query-builder to eager-load the nodes that are connected to
// the "page_source" edge. The optional arguments are used to configure the query builder of the edge.
func (pq *PageQuery) WithPageSource(opts ...func(*PageSourceQuery)) *PageQuery {
	query := &PageSourceQuery{config: pq.config}
	for _, opt := range opts {
		opt(query)
	}
	pq.withPageSource = query
	return pq
}
...
func (pq *PageQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Page, error) {
	...
	if query := pq.withPageSource; query != nil {
		if err := pq.loadPageSource(ctx, query, nodes,
			func(n *Page) { n.Edges.PageSource = []*PageSource{} },
			func(n *Page, e *PageSource) { n.Edges.PageSource = append(n.Edges.PageSource, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
